...
Balancer.prototype.getRequestHandler = function (clientRequest, clientResponse) {
  ...
  return function (clientRequest, clientResponse) {  
    var route = {};
    var clientResponseEndListener = function () {
      sys.puts('ending prematurely')
      clientResponse.ended = true;
    }
    clientResponse.close = function () {
      clientResponse.removeListener("close",clientResponseEndListener); 
      clientResponse.removeListener("end",clientResponseEndListener); 
      clientResponse.removeListener("error",clientResponseEndListener);
      clientResponse.ended = true; 
      clientResponse.end()
    };
    clientResponse.addListener("close", clientResponseEndListener)
    clientResponse.addListener("end", clientResponseEndListener)
    clientResponse.addListener("error", clientResponseEndListener)
    ...
    proxyRequest.addListener("response", function (response) {
      ...
      response.addListener("data", function (chunk) {
        ...
        if (!clientResponse.ended) {
          ...
          } catch(err) {sys.puts('writing failed')}
        }
      })
